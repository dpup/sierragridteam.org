...
| `MeshMap`           | `components/mesh/MeshMap.astro`           | Frame for the /mesh topology map: MapLibre canvas, loading state, no-WebGL fallback, map key, basemap + data attribution. The map itself is `lib/mesh-map.ts`; corridor repeaters render as `.mesh-pin` DOM buttons and the `.mesh-pop` "anchored strip" popover is built in `mesh-map.ts`; both are styled in `styles/mesh.css`. |
| `MeshSidebar`       | `components/mesh/MeshSidebar.astro`       | /mesh panel skeleton (h1, window selector, honesty notes). Tiles/legend/roster are client-rendered by `lib/mesh-view.ts`.                                                                                                                                                                                                         |
