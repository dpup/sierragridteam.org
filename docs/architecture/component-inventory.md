...
| `Supporters`    | `components/home/Supporters.astro` | Full-width band acknowledging organizations backing the network, from `src/config/supporters.ts`. Logo tiles (1px rule, no shadow) + a green link to /donate. Renders nothing when the list is empty; a supporter's mark is used unmodified, so this is the one band carrying non-palette color. |
...
| `MeshMap`           | `components/mesh/MeshMap.astro`           | Frame for the /mesh topology map: MapLibre canvas, loading state, no-WebGL fallback. The map key and the basemap credit live in the panel foot and the page foot respectively. The map itself is `lib/mesh-map.ts`; corridor repeaters render as `.mesh-pin` DOM buttons and the `.mesh-pop` "anchored strip" popover is built in `mesh-map.ts`; both are styled in `styles/mesh.css`. |
| `MeshPanels`        | `components/mesh/MeshPanels.astro`        | /mesh status-board skeleton: the deep band (h1 + freshness), the attention slot, the two equal scrolling columns, the history slot and the page attribution. Every figure, row and chart path is client-rendered by `lib/mesh-view.ts` + `lib/mesh-history.ts`.                                                                                                                        |
