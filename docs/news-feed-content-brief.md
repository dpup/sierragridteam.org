...
4. **No claims about S.I.E.R.R.A's own network or operations** — status, coverage, performance, plans. This includes _implied and comparative_ claims: "where satellite struggles, a mesh like ours works," or that our network reaches the canyons and homes where people live. If a sentence would let a reader infer our coverage, reach, or reliability anywhere specific — however it's framed, and even when the ostensible subject is another technology — it's out. You cannot verify it; humans publish that news.
5. **No unverifiable claims of any kind.** Every factual claim needs a linkable source or organizational facts stated in this brief. **Hard specifics carry extra weight:** every concrete figure — version numbers, intervals, hop counts, frequencies, acreage, dates, prices — must be directly supported by the primary source you link, and you must quote the exact figure and its source in your writer notes so the reviewer can spot-check it. A plausible-sounding number is the easiest thing to get confidently wrong and the hardest for a reviewer to catch. When the source gives a range or is vague, match its precision — don't invent a sharper number than it states.
